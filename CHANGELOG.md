...
<h3>0.4.1</h3>
<p>Added a per-rule ntfy Priority setting (Min/Low/Default/High/Max), so individual detection rules can control vibration/sound behavior on ntfy independently of everything else - left on "Auto" (the default), it behaves exactly as before (your rule's Critical Alert flag maps to max priority, otherwise default). Also documented a self-hosted ntfy + iOS gotcha: iOS notifications may not arrive at all (though messages still show up when you manually open the app) unless your server has <code>upstream-base-url</code> configured and the iOS app's Default Server setting matches your server's <code>base-url</code> exactly - see the README for the full explanation and fix. No plugin change was needed for that part; it's a server/app configuration step specific to how Apple's push system works with self-hosted servers.</p>
